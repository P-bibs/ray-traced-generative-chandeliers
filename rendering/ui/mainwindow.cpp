...
#define BIND(b)                                                                                    \
    ...
    BIND(ChoiceBinding::bindRadioButtons(brushButtonGroup,
                                         NUM_BRUSH_TYPES,
                                         settings.brushType,
    ...
    // Filter dock
    BIND(ChoiceBinding::bindRadioButtons(filterButtonGroup,
                                         NUM_FILTER_TYPES,
                                         settings.filterType,
    ...
    BIND(FloatBinding::bindSliderAndTextbox(ui->edgeDetectSensitivitySlider,
                                            ui->edgeDetectSensitivityTextbox,
    ...
    BIND(ChoiceBinding::bindRadioButtons(shapesButtonGroup,
                                         NUM_SHAPE_TYPES,
                                         settings.shapeType,
                                         ui->shapeTypeCube,
                                         ui->shapeTypeCone,
    ...
    BIND(IntBinding::bindSliderAndTextbox(ui->shapeParameterSlider1,
                                          ui->shapeParameterTextbox1,
                                          settings.shapeParameter1,
                                          1.f,
                                          100.f))
    BIND(IntBinding::bindSliderAndTextbox(ui->shapeParameterSlider2,
                                          ui->shapeParameterTextbox2,
                                          settings.shapeParameter2,
                                          1.f,
                                          100.f))
    BIND(FloatBinding::bindSliderAndTextbox(ui->shapeParameterSlider3,
                                            ui->shapeParameterTextbox3,
    ...
    initializeCamtransFrustum(); // always set the viewing frustum to reasonable settings when we
                                 // start the program
...
void MainWindow::updateAspectRatio() {
    ...
    float aspectRatio =
        static_cast<float>(activeTabSize.width()) / static_cast<float>(activeTabSize.height());
...
void MainWindow::fileNew() { ui->canvas2D->newImage(); }
...
void MainWindow::fileOpen() {
    ...
    if (!file.isNull()) {
        if (file.endsWith(".xml")) {
            m_sceneParser = std::unique_ptr<CS123XmlSceneParser>(
                new CS123XmlSceneParser(file.toLatin1().data()));
...
void MainWindow::checkAllRayFeatures() { setAllRayFeatures(true); }

void MainWindow::uncheckAllRayFeatures() { setAllRayFeatures(false); }
...
void MainWindow::renderImage() {
    ...
    if (glScene) {
        ...
        auto rayScene = new RayScene(*glScene);
        ...
        // Render the image
        QSize activeTabSize = ui->tabWidget->currentWidget()->size();
        // ui->canvas2D->renderImage(m_canvas3D->getCamera(), activeTabSize.width(),
        //      activeTabSize.height());
        CS123SceneCameraData camera;
...
void MainWindow::activateCanvas2D() { ui->tabWidget->setCurrentWidget(ui->tab2D); }

void MainWindow::activateCanvas3D() { ui->tabWidget->setCurrentWidget(ui->tab3D); }

void MainWindow::clearImage() { ui->canvas2D->clearImage(); }

void MainWindow::revertImage() { ui->canvas2D->revertImage(); }

void MainWindow::setCameraAxisX() { m_canvas3D->setCameraAxisX(); }

void MainWindow::setCameraAxisY() { m_canvas3D->setCameraAxisY(); }

void MainWindow::setCameraAxisZ() { m_canvas3D->setCameraAxisZ(); }

void MainWindow::updateCameraTranslation() { m_canvas3D->updateCameraTranslation(); }

void MainWindow::updateCameraRotationN() { m_canvas3D->updateCameraRotationN(); }

void MainWindow::updateCameraRotationV() { m_canvas3D->updateCameraRotationV(); }

void MainWindow::updateCameraRotationU() { m_canvas3D->updateCameraRotationU(); }

void MainWindow::resetUpVector() { m_canvas3D->resetUpVector(); }
...
void MainWindow::updateCameraClip() { m_canvas3D->updateCameraClip(); }

void MainWindow::updateCameraHeightAngle() { m_canvas3D->updateCameraHeightAngle(); }

void MainWindow::setCameraAxonometric() { m_canvas3D->setCameraAxonometric(); }
