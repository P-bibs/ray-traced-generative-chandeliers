...
SupportCanvas2D::~SupportCanvas2D() { delete m_image; }
...
void SupportCanvas2D::resize(int width, int height) {
    ...
    delete m_image;
    m_image = new QImage(
        width, height, QImage::Format_RGBX8888 /* this corresponds to the RGBA struct */);

    // set the new image to black
    memset(m_image->bits(), 0, width * height * sizeof(RGBA));
    for (int i = sizeof(RGBA) - 1; (unsigned long)i < width * height * sizeof(RGBA);
         i += sizeof(RGBA)) {
        m_image->bits()[i] = 255;
    }
...
bool SupportCanvas2D::saveImage(const QString &file) { return m_image->save(file, nullptr, -1); }
...
void SupportCanvas2D::clearImage() { resize(width(), height()); }
