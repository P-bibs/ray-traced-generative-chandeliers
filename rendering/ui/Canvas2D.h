...
class Canvas2D : public SupportCanvas2D {
  ...
  private:
    ...
    // BRUSH MEMBER VARIABLES AND METHODS
    std::unique_ptr<Brush> initializeBrush(int brushType);
    std::unique_ptr<Brush> m_brush;
    int m_currentRadius;
    RGBA m_currentColor;
    int m_currentBrushType;

    // INTERSECT MEMBER VARIABLES AND METHODS:

    // RAY MEMBER VARIABLES AND METHODS;
    std::vector<std::unique_ptr<QImage>> m_textures;
};
