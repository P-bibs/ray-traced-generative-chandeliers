...
void ShapesScene::settingsChanged() {
    ...
    if (settings.shapeType != m_currentShapeType || settings.shapeParameter1 != m_currentParam1 || settings.shapeParameter2 != m_currentParam2) {
        m_currentShapeType = settings.shapeType;
        m_currentParam1 = settings.shapeParameter1;
        m_currentParam2 = settings.shapeParameter2;

        if (m_currentShapeType == SHAPE_CUBE) {
            m_shape = std::make_unique<Cube>(m_currentParam1, m_currentParam2);
        } else if (m_currentShapeType == SHAPE_CONE) {
            m_shape = std::make_unique<Cone>(m_currentParam1, m_currentParam2);
        } else if (m_currentShapeType == SHAPE_CYLINDER) {
            m_shape = std::make_unique<Cylinder>(m_currentParam1, m_currentParam2);
        } else if (m_currentShapeType == SHAPE_SPHERE) {
            m_shape = std::make_unique<Sphere>(m_currentParam1, m_currentParam2);
        } else {
            m_shape = std::make_unique<Cube>(m_currentParam1, m_currentParam2);
        }
    }
